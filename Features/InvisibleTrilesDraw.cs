...
namespace FEZUG.Features
{
	internal class ComponentInfoCommand : IFezugCommand
	{
		...
		public string Name => "component_old";

		public string HelpText => "Get a property of a named component.";
        ...
        public List<String> GetFieldsOf(IGameComponent component)
        {
		...
		public bool Execute(string[] args)
		{
			...
			if (args.Length < 2)
            {
                ...
                FezugConsole.Print("Fields:");
                FezugConsole.Print(String.Join(", ", GetFieldsOf(component)));
            }

            var field = component.GetType().GetFields(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic).FirstOrDefault(f => f.Name.ToLower().Equals(args[1]));
            if (field == null) {
				FezugConsole.Print("Field on component " + component.GetType().Name + " does not exist: \"" + args[1] + "\"", FezugConsole.OutputType.Error);
				return false;
			}

            FezugConsole.Print(component.GetType().Name + "." + field.Name + " = " + component_type.GetField(field.Name, BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic).GetValue(component));

			return true;
		}
	}

	internal class ComponentInfoCommandNew : IFezugCommand
	{
		public ComponentInfoCommandNew() { }
